...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def request_log(self):
        ...
        self.requestlogger.info("REQUEST:\tserver_timestamp=[%s], user_agent=[%s], path=[%s], params=[%s], token=[%s], ip=[%s], version=[%s], device_type=[%s], device_identity=[%s], client_timestamp=[%s]",
                                int(time.time())*1000, user_agent, request_path, paramStr, token, self.request.remote_ip, version, device_type, device_identity, client_timestamp)
